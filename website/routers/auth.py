...
async def change_password(
    ...
    updated_user = schemas.UserUpdate(username=user.username, new_password=bcrypt.hash(user.new_password),
                                      old_password="")
...
@auth.get('/auth/current-user')
async def ret_current_user(
        current_user: schemas.User = Depends(get_current_user_optional)
):
    if current_user is not None:
        return {'status': 1, 'username': current_user.username, 'uuid': current_user.uuid}
    else:
        return {'status': 0}
